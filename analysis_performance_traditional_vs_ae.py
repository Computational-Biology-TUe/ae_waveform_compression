...
from scipy.stats import shapiro, levene, friedmanchisquare, ttest_rel, wilcoxon
...
qss_per_sample.append(qs_per_sample)
...
for args in [args_ls20, args_ls10]:
    ...
    qss_per_sample.append(qs_samples)
...
for errors_per_sample in [rmses_per_sample, qss_per_sample]:
    # Step 1: Stack data for easier manipulation
    data = np.vstack(errors_per_sample).T  # shape (n_samples, 3)
    ...
    for i in range(n_models):
        for j in range(i+1, n_models):
            diff = data[:, i] - data[:, j]
            stat, p = shapiro(diff)
            print(f"Shapiro test for normality of RMSE difference {model_names[i]} - {model_names[j]}: p={p:.4f}")
            if p < 0.05:
                normal = False

    # Step 3: Test for equal variances (Levene’s test)
    stat, p = levene(*[data[:, i] for i in range(n_models)])
    print(f"\nLevene’s test for equal variances across models: p={p:.4f}")
    equal_variances = p >= 0.05

    # Step 4: Choose and perform global test
    if normal and equal_variances:
        print("\nRunning repeated-measures ANOVA...")
        # Convert to long format
        df_long = pd.DataFrame({
            'subject': np.repeat(np.arange(data.shape[0]), n_models),
            'model': np.tile(model_names, data.shape[0]),
            'rmse': data.flatten()
        })
        aovrm = AnovaRM(df_long, depvar='rmse', subject='subject', within=['model'])
        res = aovrm.fit()
        print(res.summary())
        p_global = res.anova_table['Pr > F'][0]
    else:
        print("\nRunning Friedman test (non-parametric)...")
        stat, p_global = friedmanchisquare(*[data[:, i] for i in range(n_models)])
        print(f"Friedman test: chi2={stat:.3f}, p={p_global:.4f}")

    # Step 5: If significant, do post-hoc testing
    if p_global < 0.05:
        print("\nGlobal test is significant. Performing post-hoc pairwise tests...")
        ...
        for i in range(n_models):
            for j in range(i+1, n_models):
                ...
                if normal:
                    ...
                    print(f"Paired t-test {label}: p={p:.4f}")
                else:
                    stat, p = wilcoxon(x, y)
                    print(f"Wilcoxon test {label}: p={p:.4f}")
                pairwise_pvals.append(p)

        # Multiple testing correction
        _, pvals_corrected, _, _ = multipletests(pairwise_pvals, method='holm')

        print("\nCorrected p-values (Holm):")
        for label, p_corr in zip(pairwise_labels, pvals_corrected):
